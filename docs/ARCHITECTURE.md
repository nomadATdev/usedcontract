...
v0.3.2 implements the IR, collector, basic inference and compatibility engine. The supplied agents are transport SDKs for emitting semantic events. They are NOT yet transparent bytecode/AST/CLR taint agents. That work is intentionally separate so the core is not coupled to one language.
