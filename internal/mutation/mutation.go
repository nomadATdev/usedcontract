	...
	for _, r := range reqs {
		...
		if r.NonNull {
			if m, ok := clone(root); ok && setPath(m, r.Path, nil) {
				b, _ := json.Marshal(m)
				out = append(out, Case{fmt.Sprintf("%s:%s:null", r.Consumer.Name, r.Path), r.Consumer.Name, r.Path, "null", b})
			}
		}
		for op, vals := range r.Comparisons {
			if op == "eq" && len(vals) > 0 {
				if m, ok := clone(root); ok && setPath(m, r.Path, "__USEDCONTRACT_UNSEEN__") {
					b, _ := json.Marshal(m)
					out = append(out, Case{fmt.Sprintf("%s:%s:unseen-equality", r.Consumer.Name, r.Path), r.Consumer.Name, r.Path, "unseen_equality", b})
				}
			}
		}
	}
...
func deletePath(root any, path string) bool {
	ps := parts(path)
	if len(ps) == 0 {
		return false
	}
	...
	for _, x := range ps[:len(ps)-1] {
		n, ok := cur[x].(map[string]any)
		if !ok {
			return false
		}
		cur = n
	}
	if _, ok := cur[ps[len(ps)-1]]; !ok {
		return false
	}
	delete(cur, ps[len(ps)-1])
	return true
}
func setPath(root any, path string, v any) bool {
	ps := parts(path)
	if len(ps) == 0 {
		return false
	}
